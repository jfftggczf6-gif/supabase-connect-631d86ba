                    ...
                    <div className="mt-1">
                      <input
                        ...
                        onChange={e => {
                          ...
                          if (files) {
                            const existing: File[] = Array.isArray(fileUploads[field.label]) ? (fileUploads[field.label] as unknown as File[]) : fileUploads[field.label] ? [fileUploads[field.label] as unknown as File] : [];
                          ...
                          {((Array.isArray(fileUploads[field.label]) ? fileUploads[field.label] : [fileUploads[field.label]]) as File[]).map((f: File, fi: number) => (
                            <div key={fi} className="flex items-center gap-2 p-2 border rounded-lg bg-muted/50">
                              ...
                              <button type="button" onClick={() => {
                                const arr: File[] = Array.isArray(fileUploads[field.label]) ? [...(fileUploads[field.label] as unknown as File[])] : [fileUploads[field.label] as unknown as File];
                                arr.splice(fi, 1);
                                if (arr.length === 0) {
