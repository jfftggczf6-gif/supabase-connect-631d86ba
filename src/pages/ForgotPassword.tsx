    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4 relative">
      ...
      <div className="w-full max-w-md">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-display font-bold text-foreground">{t('auth.forgot_password')}</h1>
          <p className="text-sm text-muted-foreground mt-1 text-center">{t('auth.forgot_subtitle')}</p>
        </div>
        ...
        <div className="bg-card rounded-xl border p-6">
          {sent ? (
            <div className="flex flex-col items-center gap-3 py-4">
              ...
              <p className="text-sm text-foreground text-center">{t('auth.reset_email_sent')}</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={isLoading} className="w-full py-2.5 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
                {isLoading ? <><Loader2 className="h-4 w-4 animate-spin" />{t('auth.sending')}</> : t('auth.send_link')}
              </button>
            </form>
          )}
        </div>
