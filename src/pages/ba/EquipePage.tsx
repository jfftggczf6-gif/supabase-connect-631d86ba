...
// Page Équipe BA — accessible Partner only (managing_director / owner / super_admin).
// Assemble : header (counts + bouton Inviter), table membres, dialog invite,
// section bindings IM↔Analyste.
...
export default function EquipePage() {
  ...
  const subtitle = currentOrg?.name
    ? `${currentOrg.name} · ${activeCount} membre${activeCount > 1 ? 's' : ''} actif${activeCount > 1 ? 's' : ''}${invitedCount ? ` · ${invitedCount} invitation${invitedCount > 1 ? 's' : ''} en cours` : ''}`
    : '';
