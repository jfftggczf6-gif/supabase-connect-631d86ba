...
export default function PreScreeningViewer({ data, onRegenerate, onLaunchPipeline }: PreScreeningViewerProps) {
  ...
  const classConfig: Record<string, { color: string; bg: string; border: string; icon: any }> = {
    AVANCER_DIRECTEMENT: { color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200', icon: CheckCircle2 },
    ACCOMPAGNER: { color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200', icon: Wand2 },
    COMPLETER_DABORD: { color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200', icon: AlertTriangle },
    REJETER: { color: 'text-red-700', bg: 'bg-red-50', border: 'border-red-200', icon: XCircle },
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Section 1 — Classification + Résumé */}
      <Card className={`p-6 ${cc.bg} ${cc.border} border`}>
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <Badge className={`${cc.bg} ${cc.color} border ${cc.border} text-sm font-semibold`}>
                {classLabel}
              </Badge>
            </div>
            <p className="text-sm">{classDetail}</p>
          </div>
        </div>

        {resumeExecutif && (
          <div className="pt-4 border-t border-border/50 space-y-4">
            {resumeExecutif.synthese && <p className="text-sm leading-relaxed">{resumeExecutif.synthese}</p>}
            ...
            {resumeExecutif.potentiel_estime && (
              <p className="text-xs italic text-muted-foreground">{resumeExecutif.potentiel_estime}</p>
            )}
          </div>
        )}
      </Card>

      {/* Section 2 — Qualité dossier */}
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-3">
          <FileSearch className="h-5 w-5 text-muted-foreground" />
          <h3 className="font-display font-semibold text-base">Qualité du dossier</h3>
          {qualiteDossier.niveau_preuve && (
            <Badge variant="outline" className="text-xs">{qualiteDossier.niveau_preuve}</Badge>
          )}
          {qualiteDossier.score_qualite != null && (
            <Badge variant="outline" className="text-xs">{qualiteDossier.score_qualite}/100</Badge>
          )}
        </div>
        <div className="flex items-center gap-4 mb-4">
          <span className="text-sm text-muted-foreground">Documents exploitables</span>
          <Progress value={qualiteDossier.total_documents > 0 ? (qualiteDossier.documents_exploitables / qualiteDossier.total_documents) * 100 : 0} className="h-2 flex-1 max-w-xs" />
          <span className="text-sm font-semibold">{qualiteDossier.documents_exploitables || 0}/{qualiteDossier.total_documents || 0}</span>
        </div>
        {qualiteDossier.couverture && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            {['finance', 'legal', 'commercial', 'rh'].map(key => {
              ...
              if (!c) return null;
              return (
                <div key={key} className={`rounded-lg border p-3 text-center ${c.couvert ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
                  ...
                  {c.manquants_critiques?.length > 0 && (
                    <div className="text-[10px] text-red-600 mt-1">{c.manquants_critiques.map((d: string, i: number) => <p key={i}>⊘ {d}</p>)}</div>
                  )}
                </div>
              );
            })}
        ...
        {qualiteDossier.note_qualite && <p className="text-xs text-muted-foreground">{qualiteDossier.note_qualite}</p>}
      </Card>
      ...
      <div>
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          ...
          <h3 className="font-display font-semibold text-base">Anomalies et red flags</h3>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-3">
          <FileSearch className="h-5 w-5 text-muted-foreground" />
          <h3 className="font-display font-semibold text-base">Cross-validation</h3>
        </div>
        <div>
          ...
          {cvRow('bilan', 'Bilan équilibré', crossValidation.bilan_equilibre, crossValidation.bilan_detail)}
          {cvRow('charges', 'Charges vs effectifs', crossValidation.charges_vs_effectifs, crossValidation.charges_vs_effectifs_detail)}
          {cvRow('tresorerie', 'Trésorerie cohérente', crossValidation.tresorerie_coherent, crossValidation.tresorerie_detail)}
          {cvRow('dates', 'Dates cohérentes', crossValidation.dates_coherentes, crossValidation.dates_detail)}
        </div>
      </Card>
      ...
      {/* Section 6 — Potentiel et reconstructibilité */}
      {potentiel && (
        <Card className="p-5 border-teal-200 bg-teal-50/30">
          <div className="flex items-center gap-2 mb-3">
            <Wand2 className="h-5 w-5 text-teal-600" />
            <h3 className="font-display font-semibold text-base text-teal-800">Potentiel & Reconstructibilité IA</h3>
          </div>

          {potentiel.fiabilite_pipeline_estimee != null && (
            <div className="mb-4 p-3 rounded-lg bg-teal-100/50 border border-teal-200">
              <div className="flex items-center gap-3">
                <span className="text-xs text-teal-700">Fiabilité pipeline estimée</span>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-4">
            {potentiel.donnees_fiables?.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-emerald-700 mb-1">✅ Données fiables</p>
            ...
            {potentiel.donnees_estimables_ia?.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-amber-700 mb-1 flex items-center gap-1"><Wand2 className="h-3 w-3" /> Estimable par l'IA</p>
            ...
            {potentiel.donnees_non_reconstituables?.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-red-700 mb-1">❌ Non reconstituable</p>
        ...
        <Card className="p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            {profilRisque.score_risque != null && (
              <Badge variant="outline" className="text-xs">Sûreté : {profilRisque.score_risque}/100</Badge>
            )}
          </div>
          <div className="space-y-2">
        ...
        <Card className="p-5">
          ...
          <div className="space-y-2">
            {planAction.sort((a: any, b: any) => (a.priorite || 5) - (b.priorite || 5)).map((r: any, i: number) => (
              <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border ${r.bloquant_pipeline ? 'border-red-200 bg-red-50/50' : 'border-border bg-muted/30'}`}>
      ...
      {/* Section 9 — Recommandation pipeline */}
      {recommandationPipeline && (
        <Card className={`p-5 border ${recommandationPipeline.lancer_pipeline ? 'border-emerald-200 bg-emerald-50/30' : 'border-amber-200 bg-amber-50/30'}`}>
          <div className="flex items-center gap-2 mb-3">
            <Rocket className="h-5 w-5 text-muted-foreground" />
            <h3 className="font-display font-semibold text-base">
              {recommandationPipeline.lancer_pipeline ? 'Dossier prêt pour le pipeline' : 'Pipeline non recommandé pour l\'instant'}
            </h3>
          </div>
          <p className="text-sm mb-3">{recommandationPipeline.raison}</p>

          {recommandationPipeline.avertissement && (
            <div className="p-2 rounded bg-amber-100 border border-amber-200 mb-3">
              <p className="text-xs text-amber-700">⚠️ {recommandationPipeline.avertissement}</p>
            </div>
          )}

          {recommandationPipeline.modules_pertinents?.length > 0 && (
            <div className="mb-3">
              <p className="text-xs font-semibold mb-1">Modules pertinents :</p>
              <div className="flex flex-wrap gap-1.5">
                {recommandationPipeline.modules_pertinents.map((m: string, i: number) => (
                  <Badge key={i} variant="outline" className="text-xs bg-emerald-50 text-emerald-700 border-emerald-200">{m}</Badge>
                ))}
              </div>
            </div>
          )}
          {recommandationPipeline.modules_inutiles?.length > 0 && (
            <div className="mb-3">
              <p className="text-xs font-semibold mb-1">Modules insuffisamment alimentés :</p>
              <div className="flex flex-wrap gap-1.5">
                {recommandationPipeline.modules_inutiles.map((m: string, i: number) => (
                  <Badge key={i} variant="outline" className="text-xs bg-red-50 text-red-700 border-red-200">{m}</Badge>
                ))}
              </div>
            </div>
          )}
          ...
          {onLaunchPipeline && (
            <div className="flex gap-2 pt-2">
              {recommandationPipeline.lancer_pipeline ? (
                <Button onClick={onLaunchPipeline} className="gap-2">
                  <Rocket className="h-4 w-4" /> Lancer le pipeline complet
                </Button>
              ) : (
                <Button variant="outline" onClick={onLaunchPipeline} className="gap-2 opacity-70">
      ...
      {/* Section 10 — Pathway + Programme match */}
      {pathway && (
        <Card className="p-5">
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className="space-y-2">
            {programmeMatch.criteres_ok?.map((c: any, i: number) => (
              <div key={i}>
                ...
                {typeof c === 'object' && c.detail && <p className="text-[10px] text-muted-foreground pl-5">{c.detail}</p>}
              </div>
            ))}
            {programmeMatch.criteres_ko?.map((c: any, i: number) => (
              <div key={i}>
                ...
                {typeof c === 'object' && c.detail && <p className="text-[10px] text-muted-foreground pl-5">{c.detail}</p>}
                {typeof c === 'object' && c.comment_corriger && <p className="text-[10px] text-primary pl-5">💡 {c.comment_corriger}</p>}
              </div>
            ))}
          </div>
