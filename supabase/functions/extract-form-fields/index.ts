...
serve(async (req) => {
  ...
  try {
    ...
    // Check role (legacy + org) avec tri par priorité
    const [{ data: roleData }, { data: orgMems }] = await Promise.all([
      supabase.from("user_roles").select("role").eq("user_id", user.id).maybeSingle(),
      supabase.from("organization_members").select("role").eq("user_id", user.id).eq("is_active", true),
    ]);
    ...
    const aiResponse = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 8192, // marge anti-troncature (Sonnet 4.6 verbeux, liste de champs) — ex-4096
        temperature: 0,
